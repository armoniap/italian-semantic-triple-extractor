...
class OptimizedPerformanceTester {
   ...
   */
  async testOptimizedAPICallPerformance(texts: string[]): Promise<OptimizedPerformanceMetrics> {
    ...
    const { metrics } = await this.measurePerformance(
      ...
      async () => {
        ...
        const results = await this.optimizedGeminiService.processBatch(requests);
        return results;
      },
      totalTokens
    );
   ...
   */
  async testOptimizedEntityExtractionPerformance(text: string): Promise<OptimizedPerformanceMetrics> {
    console.log('🚀 Testing Optimized Entity Extraction...');
    
    const { metrics } = await this.measurePerformance(
      'OPTIMIZED_ENTITY_EXTRACTION',
      async () => {
        const result = await this.optimizedEntityExtractor.extractEntities(text);
        return result;
      },
      text.length / 4
    );
   ...
   */
  async testCacheEffectiveness(text: string, iterations: number = 3): Promise<OptimizedPerformanceMetrics> {
    ...
    const { metrics } = await this.measurePerformance(
      ...
      async () => {
        ...
        for (let i = 0; i < iterations; i++) {
          ...
          const result = await this.optimizedGeminiService.analyzeItalianText(text, 'entities');
          results.push(result);
        }
   ...
   */
  async testParallelProcessingEfficiency(texts: string[]): Promise<OptimizedPerformanceMetrics> {
    ...
    metrics.parallelEfficiency = (metrics as any).result?.parallelEfficiency || 1;
   ...
   */
  async testMemoryOptimization(texts: string[]): Promise<OptimizedPerformanceMetrics> {
    ...
    const { metrics } = await this.measurePerformance(
      ...
      async () => {
        ...
        return {
          ...
          avgMemoryPerRequest: memoryMeasurements.reduce((sum, m) => sum + m.memoryDelta, 0) / memoryMeasurements.length
        };
      },
      totalTokens
    );
   ...
   */
  async runOptimizedTestSuite(config: typeof TEST_CONFIG): Promise<OptimizedPerformanceMetrics[]> {
    ...
    try {
      ...
      await this.testOptimizedEntityExtractionPerformance(config.testTexts.medium);
  ...
  generateOptimizedReport(): string {
    const report = ['📊 OPTIMIZED PERFORMANCE TEST RESULTS', '=' .repeat(55), ''];
    ...
    this.metrics.forEach(metric => {
      ...
      if (metric.parallelEfficiency !== undefined) {
        report.push(`   ⚡ Parallel Efficiency: ${metric.parallelEfficiency.toFixed(2)}x`);
      }
    ...
    const avgThroughput = this.metrics
      .filter(m => m.throughputPerSecond)
      .reduce((sum, m) => sum + (m.throughputPerSecond || 0), 0) / this.metrics.filter(m => m.throughputPerSecond).length;
    ...
    report.push(`   Average Throughput: ${avgThroughput.toFixed(2)} tokens/sec`);
    
    // Optimization insights
    const parallelEfficiencyMetrics = this.metrics.filter(m => m.parallelEfficiency);
    if (parallelEfficiencyMetrics.length > 0) {
      const avgParallelEfficiency = parallelEfficiencyMetrics.reduce((sum, m) => sum + (m.parallelEfficiency || 1), 0) / parallelEfficiencyMetrics.length;
      report.push(`   Average Parallel Efficiency: ${avgParallelEfficiency.toFixed(2)}x speedup`);
    }
  ...
  compareWithBaseline(baselineMetrics: any[]): string {
    const comparison = ['🆚 OPTIMIZATION COMPARISON REPORT', '=' .repeat(55), ''];
    ...
    this.metrics.forEach(optimizedMetric => {
      ...
      const baselineMetric = baselineMetrics.find(m => m.name === baselineMetricName);
      
      if (baselineMetric) {
        const timeImprovement = ((baselineMetric.executionTime - optimizedMetric.executionTime) / baselineMetric.executionTime) * 100;
        const memoryImprovement = ((baselineMetric.memoryUsed - optimizedMetric.memoryUsed) / baselineMetric.memoryUsed) * 100;
        const throughputImprovement = optimizedMetric.throughputPerSecond && baselineMetric.throughputPerSecond
          ? ((optimizedMetric.throughputPerSecond - baselineMetric.throughputPerSecond) / baselineMetric.throughputPerSecond) * 100
          : 0;

        comparison.push(`🔸 ${optimizedMetric.name} vs ${baselineMetric.name}:`);
        comparison.push(`   ⏱️  Time: ${baselineMetric.executionTime.toFixed(2)}ms → ${optimizedMetric.executionTime.toFixed(2)}ms`);
        comparison.push(`      📈 ${timeImprovement > 0 ? '✅' : '❌'} ${Math.abs(timeImprovement).toFixed(1)}% ${timeImprovement > 0 ? 'improvement' : 'regression'}`);
        
        comparison.push(`   🧠 Memory: ${baselineMetric.memoryUsed.toFixed(2)}MB → ${optimizedMetric.memoryUsed.toFixed(2)}MB`);
        comparison.push(`      📈 ${memoryImprovement > 0 ? '✅' : '❌'} ${Math.abs(memoryImprovement).toFixed(1)}% ${memoryImprovement > 0 ? 'improvement' : 'regression'}`);
        
        if (throughputImprovement > 0) {
          comparison.push(`   🚀 Throughput: ${baselineMetric.throughputPerSecond?.toFixed(2)} → ${optimizedMetric.throughputPerSecond?.toFixed(2)} tokens/sec`);
          comparison.push(`      📈 ✅ ${throughputImprovement.toFixed(1)}% improvement`);
        }
