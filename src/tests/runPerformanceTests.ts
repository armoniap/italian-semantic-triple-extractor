...
class PerformanceRunner {
  ...
  generateComparisonReport(): ComparisonResult[] {
    if (this.baselineResults.length === 0) {
      throw new Error(
        'No baseline results available. Run baseline tests first.'
      );
    }

    if (this.optimizedResults.length === 0) {
      throw new Error(
        'No optimized results available. Run optimized tests first.'
      );
    }
    ...
    this.baselineResults.forEach(baseline => {
      const optimized = this.optimizedResults.find(
        opt => opt.name === baseline.name
      );

      if (optimized) {
        const timeImprovement =
          ((baseline.executionTime - optimized.executionTime) /
            baseline.executionTime) *
          100;
        const memoryImprovement =
          ((baseline.memoryUsed - optimized.memoryUsed) / baseline.memoryUsed) *
          100;
        const throughputImprovement =
          baseline.throughputPerSecond && optimized.throughputPerSecond
            ? ((optimized.throughputPerSecond - baseline.throughputPerSecond) /
  ...
  printComparisonReport(): void {
    ...
    comparisons.forEach(comparison => {
      console.log(`\n🔸 ${comparison.testName}:`);
      console.log(
        `  ⏱️  Time: ${comparison.beforeMetrics.executionTime.toFixed(2)}ms → ${comparison.afterMetrics?.executionTime.toFixed(2)}ms`
      );
      console.log(
        `     📈 ${comparison.improvements.timeImprovement > 0 ? '✅' : '❌'} ${comparison.improvements.timeImprovement.toFixed(1)}% improvement`
      );

      console.log(
        `  🧠 Memory: ${comparison.beforeMetrics.memoryUsed.toFixed(2)}MB → ${comparison.afterMetrics?.memoryUsed.toFixed(2)}MB`
      );
      console.log(
        `     📈 ${comparison.improvements.memoryImprovement > 0 ? '✅' : '❌'} ${comparison.improvements.memoryImprovement.toFixed(1)}% improvement`
      );

      if (comparison.improvements.throughputImprovement !== undefined) {
        console.log(
          `  🚀 Throughput: ${comparison.beforeMetrics.throughput?.toFixed(2)} → ${comparison.afterMetrics?.throughput?.toFixed(2)} tokens/sec`
        );
        console.log(
          `     📈 ${comparison.improvements.throughputImprovement > 0 ? '✅' : '❌'} ${comparison.improvements.throughputImprovement.toFixed(1)}% improvement`
        );
      }
    });
    ...
    const avgTimeImprovement =
      comparisons.reduce((sum, c) => sum + c.improvements.timeImprovement, 0) /
      comparisons.length;
    const avgMemoryImprovement =
      comparisons.reduce(
        (sum, c) => sum + c.improvements.memoryImprovement,
        0
      ) / comparisons.length;

    console.log('\n📊 OVERALL IMPROVEMENTS:');
    console.log(
      `  ⏱️  Average Time Improvement: ${avgTimeImprovement.toFixed(1)}%`
    );
    console.log(
      `  🧠 Average Memory Improvement: ${avgMemoryImprovement.toFixed(1)}%`
    );

    const throughputComparisons = comparisons.filter(
      c => c.improvements.throughputImprovement !== undefined
    );
    if (throughputComparisons.length > 0) {
      const avgThroughputImprovement =
        throughputComparisons.reduce(
          (sum, c) => sum + (c.improvements.throughputImprovement || 0),
          0
        ) / throughputComparisons.length;
      console.log(
        `  🚀 Average Throughput Improvement: ${avgThroughputImprovement.toFixed(1)}%`
      );
    }
  }
...
async function main() {
  ...
  if (!apiKey) {
    console.error(
      '❌ Please provide GEMINI_API_KEY environment variable or as argument'
    );
    process.exit(1);
  }
  ...
  try {
    switch (command) {
      ...
      default:
        ...
        console.log(
          '  npm run perf full <api-key>      - Run both and compare'
        );
