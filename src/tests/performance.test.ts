...
class PerformanceTester {
    ...
    const { metrics } = await this.measurePerformance(
      ...
      async () => {
        ...
        for (const text of texts) {
          const result = await this.geminiService.analyzeItalianText(
            text,
            'entities'
          );
          results.push(result);
   ...
   */
  async testTextPreprocessingPerformance(
    texts: string[]
   ...
   */
  async testEntityExtractionPerformance(
    text: string
  ): Promise<PerformanceMetrics> {
   ...
   */
  async testTripleExtractionPerformance(
    text: string
  ): Promise<PerformanceMetrics> {
  ...
  async testEndToEndPerformance(text: string): Promise<PerformanceMetrics> {
    ...
    const { metrics } = await this.measurePerformance(
      ...
      async () => {
        ...
        const triples = await this.tripleExtractor.extractTriples(
          text,
          entities.entities
        );
...
export const TEST_CONFIG: TestConfiguration = {
  ...
  testTexts: {
    small:
      'Leonardo da Vinci nacque a Vinci nel 1452. Era un genio del Rinascimento italiano.',
