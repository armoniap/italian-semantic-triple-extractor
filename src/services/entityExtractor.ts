...
import { ItalianEntity, ItalianEntityType, EntityExtractionResult, ItalianEntityMetadata } from '@/types/entities';
import { GeminiAPIService } from './geminiAPI';

interface ItalianGeographicData {
  cities: Record<string, { region: string; province: string; population?: number; coordinates?: [number, number] }>;
  regions: Record<string, { capital: string; area?: number; population?: number }>;
...
export class ItalianEntityExtractor {
  ...
  async extractEntities(text: string): Promise<EntityExtractionResult> {
    ...
    try {
      ...
      if (language !== 'it') {
        console.warn('Text may not be in Italian, extraction accuracy may be reduced');
      }

      // Get AI-based extraction
      const aiResult = await this.geminiService.analyzeItalianText(preprocessedText, 'entities');
      
      // Apply rule-based enhancement for Italian-specific patterns
      const ruleBasedEntities = this.extractRuleBasedItalianEntities(preprocessedText);
      
      // Merge and deduplicate entities
      const mergedEntities = this.mergeEntityResults(aiResult.entities || [], ruleBasedEntities, text);
      ...
      const scoredEntities = this.applyItalianConfidenceScoring(enrichedEntities);
  ...
  private createItalianEntity(rawEntity: any, index: number, originalText: string): ItalianEntity | null {
    try {
      ...
      const metadata = this.extractEntityMetadata(rawEntity.metadata, entityType);
  ...
  private findEntityPosition(entityText: string, originalText: string): { start: number; end: number } {
  ...
  private normalizeEntityType(rawType: string): ItalianEntityType {
    ...
    const normalizedType = typeMapping[rawType?.toUpperCase()] || ItalianEntityType.MISCELLANEOUS;
    return normalizedType;
  }

  private extractEntityMetadata(rawMetadata: any, entityType: ItalianEntityType): ItalianEntityMetadata {
    ...
    if (rawMetadata) {
      ...
      if (rawMetadata.coordinates) metadata.coordinates = rawMetadata.coordinates;

      // Cultural context
      if (rawMetadata.culturalContext) metadata.culturalContext = rawMetadata.culturalContext;
      if (rawMetadata.historicalPeriod) metadata.historicalPeriod = rawMetadata.historicalPeriod;

      // Linguistic variants
      if (rawMetadata.dialectalVariants) metadata.dialectalVariants = rawMetadata.dialectalVariants;
  ...
  private addDefaultMetadata(metadata: ItalianEntityMetadata, entityType: ItalianEntityType): void {
  ...
  private detectLanguage(text: string): 'it' | 'unknown' {
    ...
    const italianIndicators = {
      // Common words
      words: ['il', 'la', 'di', 'che', 'e', 'a', 'un', 'per', 'in', 'con', 'non', 'una', 'su', 'del', 'da', 'al', 'come', 'le', 'si', 'nella', 'sono', 'stato', 'fatto', 'essere', 'aveva', 'anche', 'molto', 'tutto', 'però', 'degli', 'viene', 'quando', 'fare', 'solo', 'ancora'],
      ...
      // Geographic markers
      geoMarkers: ['roma', 'milano', 'napoli', 'torino', 'firenze', 'venezia', 'bologna', 'genova', 'palermo', 'italia', 'italiano', 'italiana'],
      // Cultural markers
      culturalMarkers: ['papa', 'vaticano', 'rinascimento', 'arte', 'pizza', 'pasta', 'gelato', 'calcio', 'serie']
    };
    ...
    // Check common words
    const italianWordCount = words.filter(word => italianIndicators.words.includes(word)).length;
  ...
  filterEntitiesByConfidence(entities: ItalianEntity[], minConfidence: number = 0.5): ItalianEntity[] {
    return entities.filter(entity => entity.confidence >= minConfidence);
  }

  groupEntitiesByType(entities: ItalianEntity[]): Record<ItalianEntityType, ItalianEntity[]> {
    const grouped: Record<ItalianEntityType, ItalianEntity[]> = {} as Record<ItalianEntityType, ItalianEntity[]>;
  ...
  private async enrichWithItalianData(entities: any[]): Promise<any[]> {
    return entities.map(entity => {
      ...
      if (entity.text?.toLowerCase().includes('roma')) {
        enriched.metadata = { ...enriched.metadata, dialectalVariants: ['rome'] };
      }
  ...
  private isKnownItalianEntity(text: string, type: string): boolean {
    ...
    switch (type) {
      case 'ITALIAN_CITY':
        return Object.keys(this.italianGeoData.cities).some(city => 
          city.toLowerCase() === textLower
        );
      case 'ITALIAN_REGION':
        return Object.keys(this.italianGeoData.regions).some(region => 
          region.toLowerCase() === textLower
        );
      default:
        return false;
    }
  ...
  private loadItalianGeographicData(): ItalianGeographicData {
    return {
      cities: {
        ...
        'Milano': { region: 'Lombardia', province: 'Milano', population: 1366000, coordinates: [45.4642, 9.1900] },
        ...
        'Catania': { region: 'Sicilia', province: 'Catania', population: 315000, coordinates: [37.5079, 15.0830] },
      ...
      monuments: {
        ...
        'Torre di Pisa': { city: 'Pisa', region: 'Toscana', type: 'Torre campanaria' },
        'Duomo di Milano': { city: 'Milano', region: 'Lombardia', type: 'Cattedrale gotica' },
        'Ponte di Rialto': { city: 'Venezia', region: 'Veneto', type: 'Ponte storico' },
        'Fontana di Trevi': { city: 'Roma', region: 'Lazio', type: 'Fontana barocca' },
        'Ponte Vecchio': { city: 'Firenze', region: 'Toscana', type: 'Ponte medievale' },
        'Arena di Verona': { city: 'Verona', region: 'Veneto', type: 'Anfiteatro romano' }
      }
    };
  }
