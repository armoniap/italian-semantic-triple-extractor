...
const TripleList: React.FC<TripleListProps> = ({ triples }) => {
  ...
  const groupedTriples = triples.reduce((groups, triple) => {
          ...
          <div className="px-4 py-3 border-b bg-gray-50">
            <h4 className="text-sm font-medium text-gray-900 flex items-center justify-between">
              ...
              <span className="text-xs text-gray-500">
                {typeTriples.length} {typeTriples.length === 1 ? 'relazione' : 'relazioni'}
              </span>
            </h4>
          </div>
              ...
              {typeTriples.map((triple) => (
                ...
                >
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-shrink-0">
                      <div className={`px-3 py-1 rounded-full text-sm font-medium ${getPredicateColor(triple.predicate.type)}`}>
                  ...
                  {triple.source && (
                    <div className="mt-2 text-xs text-gray-500">
                      Fonte: caratteri {triple.source.startOffset}-{triple.source.endOffset}
                    </div>
                  )}
                </div>
              ))}
