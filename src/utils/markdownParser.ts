...
export class MarkdownParser {
  ...
  private static extractItalianMetadata(markdown: string): MarkdownMetadata & {
    ...
    const languageConfidence =
      this.calculateItalianLanguageConfidence(plainText);
  ...
  static highlightEntitiesInMarkdown(
    ...
    sortedEntities.forEach(entity => {
      ...
      const entityText = highlightedMarkdown.substring(
        entity.startOffset,
        entity.endOffset
      );
  ...
  private static splitIntoItalianSentences(text: string): string[] {
    ...
    for (let i = 0; i < sentences.length; i++) {
      ...
      if (
        sentence.match(/\b(dr|prof|sig|dott|ing|avv|on|sen)$/i) &&
  ...
  private static calculateItalianLanguageConfidence(text: string): number {
    ...
    const functionWordCount = words.filter(word =>
      italianFunctionWords.includes(word)
    ).length;
    ...
    const italianTerms = text.match(
      /\b(italia|roma|milano|napoli|firenze|venezia|papa|vaticano|rinascimento|arte|cultura|storia|cucina|pasta|pizza)\b/gi
    );
    if (italianTerms) {
  ...
  private static assessFormalityLevel(
    text: string
  ): 'formale' | 'informale' | 'accademico' | 'misto' {
    const formalMarkers =
      text.match(/\b(egregio|spettabile|cortese|distinti|saluti)\b/gi) || [];
    const informalMarkers = text.match(/\b(ciao|bello|roba|tipo)\b/gi) || [];
    const academicMarkers =
      text.match(/\b(analisi|ricerca|studio|metodologia)\b/gi) || [];

    if (
      academicMarkers.length > formalMarkers.length &&
      academicMarkers.length > informalMarkers.length
    ) {
      return 'accademico';
